...
class Solution:
    def PossibleWords(self, arr):
        res = []
        padmap = ["", "", "abc", "def", "ghi", "jkl", "mno", "pqrs", "tuv", "wxyz"]
        self.possibleWordsrecr(arr, 0, "", padmap, res)
        return res

    def possibleWordsrecr(self,arr, index, prefix, padmap, res):
        if index == len(arr):
            res.append(prefix)
            return 
        digit = arr[index]
        if digit < 2 or digit > 9:
            self.possibleWordsrecr(arr, index+1, prefix, padmap, res)
            return 
        
        for ch in padmap[digit]:
            self.possibleWordsrecr(arr, index+1, prefix+ch, padmap, res)
            
arr = [5,2]
ob = Solution()
print("The result is:")
print(ob.PossibleWords(arr))
